...
| [SITE-ROTATE](site.md#site-rotate)   | partial | —       | [rotate-key.yml](../.github/workflows/rotate-key.yml), [rotate-key.t](../t/ci/rotate-key.t). The caller pins the reusable workflow of FuguWeb to the commit of the v0.6.2 tag, and the callee holds SITE-ROTATE-5 at that commit. Absent: the `ADMIN` secrets and variables of SITE-ROTATE-1, because no `admin` mint has run. The organization holds `RELENG_ROOT_KEY_A`, `RELENG_RELEASE_KEY_A`, `RELENG_ROOT_SLOT` and `RELENG_RELEASE_SLOT`. Absent: `RELENG_ROOT_KEY_B` and `RELENG_RELEASE_KEY_B` of SITE-ROTATE-1. Absent: `ADMIN_APP_ID` and `ADMIN_APP_PRIVATE_KEY` of SITE-ROTATE-4, which the operator writes by hand. The `releng` environment holds `RELENG_APP_ID` and `RELENG_APP_PRIVATE_KEY`. Tooling declares `fugureleng-1-release` in `deps/KEYS.txt` and in `org/sync/deps/KEYS.txt`, per SITE-ROTATE-11 and SITE-ROTATE-21. An `admin` key signs no release, so no `admin` key takes a declaration. The operator writes each declaration by hand. Absent: the read of the callee outputs. FuguWeb WEB-ACTIONS-2 and WEB-ACTIONS-10 give the caller that work, and the caller holds no step, per SITE-ROTATE-14. Absent: the deployment-branch rule of the `admin` environment, per SITE-ROTATE-27. The `releng` environment holds its rule, and the operator sets each one. |
