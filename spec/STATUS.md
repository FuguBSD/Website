...
| [SITE-KEYS](site.md#site-keys)       | partial | —       | [rotate-key.yml](../.github/workflows/rotate-key.yml) derives each value that a directory decides, and the org word of SITE-KEYS-6 is one of them: `fugureleng` for `releng`, and `fuguadmin` for `admin`. Absent: SITE-KEYS-1, SITE-KEYS-2, SITE-KEYS-3, SITE-KEYS-4, SITE-KEYS-5 and SITE-KEYS-7, because each one needs a key directory that stands. `.fuguwebrc` holds no `keys` block, and a bootstrap mint writes the block, the first key and the manifest of one directory in one commit. The repository holds no `web/keys` directory, and no rule of this specification names one. SITE-KEYS-7 keeps each key file of `web/releng` and `web/admin` only. No file therefore answers `https://www.fugubsd.org/keys/fugubsd-1-release.pub`, and `deps/KEYS.txt` of the org pack pins that URL. Four consumers hold a `dist` entry with no recorded digest, and each one fails `make deps`: FuguBench, FuguSeed, FuguTTX and FuguVM. FuguWeb passes, because it recorded the digest of its Fugu dependency and then it released v0.6.1. |
| [SITE-ROTATE](site.md#site-rotate)   | partial | —       | [rotate-key.yml](../.github/workflows/rotate-key.yml), [rotate-key.t](../t/ci/rotate-key.t). The caller pins the reusable workflow of FuguWeb to the commit of the v0.6.1 tag, and the callee holds SITE-ROTATE-5 at that commit. Absent: SITE-ROTATE-1, because no file of this repository holds a secret. A mint writes one secret, the idle slot, and it moves the variable while the active slot is empty. Absent: SITE-ROTATE-4, because the operator writes the four App credentials: `RELENG_APP_ID`, `RELENG_APP_PRIVATE_KEY`, `ADMIN_APP_ID` and `ADMIN_APP_PRIVATE_KEY`. Absent: SITE-ROTATE-11, SITE-ROTATE-21 and SITE-ROTATE-22, the declaration of a key in FuguBSD/Tooling, which the operator writes by hand. Absent: the read of the callee outputs. FuguWeb WEB-ACTIONS-2 and WEB-ACTIONS-10 give the caller that work, and the caller holds no step, per SITE-ROTATE-14. Absent: SITE-ROTATE-27, the deployment-branch rule of each environment, which is a repository setting that the operator sets.                     |
