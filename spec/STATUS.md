...
| [SITE-KEYS](site.md#site-keys)       | partial | —       | [rotate-key.yml](../.github/workflows/rotate-key.yml) derives each value that a directory decides, and the org word of SITE-KEYS-6 is one of them: `fugureleng` for `releng`, and `fuguadmin` for `admin`. Absent: SITE-KEYS-1, SITE-KEYS-2, SITE-KEYS-3, SITE-KEYS-4, SITE-KEYS-5 and SITE-KEYS-7, because each one needs a key directory that stands. `.fuguwebrc` holds no `keys` block, and a bootstrap mint writes the block, the first key and the manifest of one directory in one commit. The operator approved one exception: no file answers `https://www.fugubsd.org/keys/fugubsd-1-release.pub`, and `deps/KEYS.txt` of the org pack pins that URL. SITE-KEYS-7 keeps each key file of the two directories above. Each consumer that holds a `dist` entry with no recorded digest therefore fails `make deps`.                                                                                                                                                                                                |
